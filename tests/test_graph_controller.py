...
def test_authorized_success_shadow_finalizes_without_replacing_live_answer_or_route():
    ...
    state = _state(
        ...
        brain_result=BrainOutcome(
            ...
            final_answer_draft=FinalAnswerDraft(text="Live Brain answer"),
        ),
    )

    update = create_controller_node(shadow_finalizer=observer)(state)
    ...
    assert update["shadow_finalization_result"].execution_summary.failed_step_ids == ()
    assert update["final_answer"] == "Live Brain answer"
...
def test_failed_terminal_transition_is_shadow_finalized_from_controller_facts():
    observer = ObservingFinalizer()
    update = create_controller_node(shadow_finalizer=observer)(_state(
    ...
    assert update["shadow_finalization_result"].execution_summary.status == ExecutionStatus.FAILED


def test_direct_response_is_shadow_finalized_and_brain_answer_remains_live():
    observer = ObservingFinalizer()
    ...
    update = create_controller_node(shadow_finalizer=observer)(_state(
        ...
        brain_result=BrainOutcome(
            outcome=BrainOutcomeKind.FINAL_ANSWER,
            final_answer_draft=FinalAnswerDraft(text="Direct Brain answer"),
        ),
    ))

    assert observer.requests[0].direct_response is True
    assert update["shadow_finalization_result"].execution_summary.completed_step_ids == ()
    assert update["final_answer"] == "Direct Brain answer"


def test_cancelled_terminal_transition_is_shadow_finalized():
    observer = ObservingFinalizer()
    state = _state()
    ...
    assert update["shadow_finalization_result"].execution_summary.status == ExecutionStatus.CANCELLED


def test_equivalent_terminal_executions_produce_equivalent_shadow_summaries():
    def run_once():
        return create_controller_node()(_state(
            execution_state=_completed_plan_state(),
            brain_result=BrainOutcome(
                outcome=BrainOutcomeKind.FINAL_ANSWER,
                final_answer_draft=FinalAnswerDraft(text="Live"),
            ),
        ))["shadow_finalization_result"].execution_summary

    assert run_once() == run_once()


def test_shadow_failure_is_observable_and_cannot_change_live_terminal_behavior():
    class FailingFinalizer:
        def finalize(self, _request):
            raise RuntimeError("shadow unavailable")

    update = create_controller_node(shadow_finalizer=FailingFinalizer())(_state(
        execution_state=_completed_plan_state(),
        brain_result=BrainOutcome(
            outcome=BrainOutcomeKind.FINAL_ANSWER,
            final_answer_draft=FinalAnswerDraft(text="Still live"),
        ),
    ))

    assert update["final_answer"] == "Still live"
    assert "shadow unavailable" in update["shadow_finalization_error"]
    assert "shadow_finalization_result" not in update
    assert update["execution_state"].protocol_visible.status == ExecutionStatus.COMPLETED
    assert update["controller_decision"].decision_type == ControllerDecisionType.DISPATCH_SUMMARY
    assert route_after_controller(update) == "__end__"


def test_non_terminal_controller_decision_does_not_invoke_shadow_finalizer():
    observer = ObservingFinalizer()
    update = create_controller_node(shadow_finalizer=observer)(_state(
